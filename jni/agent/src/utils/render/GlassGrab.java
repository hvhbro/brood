...
public final class GlassGrab {
    ...
    private static ShaderUtil copyShader, blurShader, windowShader;
    ...
    private static boolean init(GameContext ctx) {
        ...
        try {
            ...
            tw = Math.max(64, ctx.scaledWidth * scale / 2);
            th = Math.max(64, ctx.fbHeight / 2);
            copyTex = newTex(tw, th);
    ...
    public static void grab(GameContext ctx) {
        ...
        try {
            ...
            // СВОЯ ортопроекция в half-res пространстве на все 3 прохода
            ...
            GL11.glPopMatrix();
            bindFbo(prevFbo);
            int sc = Math.max(1, Math.round(GuiScale.get(ctx)));
            GL11.glViewport(0, 0, ctx.scaledWidth * sc, ctx.fbHeight);
    ...
    private static void drawFs(ShaderUtil sh, int w, int h) {
        sh.start();
        sh.uniformI("tex", 0);
        drawFsPass(sh, w, h);
        sh.stop();
